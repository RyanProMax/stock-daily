...
- 解读：顶部总览只读取行情；Codex Agent 逐条只读取对应的原始标题与核验事实，输出中文标题、摘要、方向、传导链、板块和有来源支持的 ticker。“中性”仅用于例行事项、整体稳定或正负因素抵消；信息不足类措辞会被质量门拒绝。最多修正三轮，连续未通过质量门则整次失败。
...
- 溯源元数据：Agent 模型版本；
- 精简 JSON：一条结构化 AI 总览、六条中美行情、CN/US 各三条一级行业热度、工作日每个市场 4–6 条新闻或周末 3–6 条新闻及英文翻译；
- 新闻必要字段：分类、优先级、标题、摘要、原始标题、最小核验事实、来源 URL、来源名、发布时间、AI 方向与传导解读、板块、已验证 ticker。
