...
一个部署在 Cloudflare Pages、使用 Cloudflare D1 保存历史日报和周报的双语市场备忘录。页面只保留“AI 总览 → 中美市场状态 → 重点新闻 → 其他日报”四层核心信息。
...
1. 获取 S&P 500、NASDAQ、DOW、美国 10 年期收益率、上证指数和沪深 300；
2. 并行读取国家统计局、Federal Reserve、SEC、EIA 等官方 RSS 与中美财经源；单一来源超时或失败不会阻断其余来源；
...
7. 交易日的 15:00 收盘版只在当日 CN 收盘数据到位后运行；数据源尚未结算时由补偿触发重试，非交易日仍刷新新闻与核验时间；
...
每周日北京时间 `20:30` 汇总当周日报，生成基准/上行/下行情景并从官方日历筛选未来一周关键事件：
